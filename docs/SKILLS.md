...
**Two things about pruning specifically.** It is **not a judgement on the claim** — a node at `hardened · g5` can be pruned, because the only question is whether an open decision still rests on it, and the body, objections and generation are all left exactly as they were. And an unassigned node is often a **binding gap rather than a dead branch**, so *bind to a decision* is offered right next to *prune*: if the node does serve something and nobody wrote it down, say which decision and it stays. Every prune records the status it cut from (`Pruned from hardened: …`), which is what lets a reopened decision put its branches back.

**When the decision itself was wrong,** this is also where you say so. A decision that stopped being the right question — too broad, badly framed, made moot — is marked `superseded`, not `settled`: it was never answered, and recording it as answered would leave a false record. Its row stays in the table with a pointer to whatever replaced it (`→ D4, D5`), and the nodes that served it are **re-bound to the successor** rather than cut. Splitting an over-broad decision in two is this same path. The one rule that matters: never delete a row to reuse its ID, or every `serves:` pointing at it silently re-aims at the new decision.
...
- **Nothing is destroyed.** Collapsed claims are `deprecated` and pruned ones are `pruned` — never deleted, and pruning is reversible if the decision reopens, because each cut records the status it came from. Decisions work the same way: one that stopped being the right question is `superseded`, its row left in place, never deleted or renumbered. The donor in a mashup is cited, not copied; a deliverable never edits the nodes it quotes. You can always trace back.
