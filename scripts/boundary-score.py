#!/usr/bin/env python3
# Frontier scorer. Derived from third-party MIT-licensed code.
# Copyright (c) 2026 AgriciDaniel (AI Marketing Hub). MIT License; this notice is retained per its terms.
# Local changes: SCOPE_ROOT = cwd (run from the scope root) instead of the script's
# parent.parent — the installed script lives in the plugin directory, not in any scope —
# and fail fast when ./wiki is missing (was: silent empty result).
# re:Harmoniz Phase B 표적 선정용 (EVOLUTION.md §4 Phase B). 스코프 디렉토리에서 실행 — wiki/만 읽는 read-only.
"""boundary-score.py — boundary-first frontier scorer.

Reads `./wiki/**/*.md` under the current working directory (the research-scope
root), builds a wikilink graph, and emits per-page boundary scores to stdout
...
has been touched recently. That is a frontier page. Low or
negative score = hub / integrated page.

Safe to run standalone (reads wiki/ only; never writes).

This script is intentionally stdout-only. There is no `--report PATH`
equivalent because the helper is small enough to pipe directly
(`./scripts/boundary-score.py --json | jq ...`) and keeping it read-only
removes a write-path attack surface.
...
# `experiment` nodes (§2, §12) are pre-registration design records and
# `deliverable` nodes (§14) are non-evolving answer snapshots, not knowledge
# nodes — excluded from the frontier graph so they never skew a claim's score.
EXCLUDE_TYPES = {"meta", "experiment", "deliverable"}
...
# code blocks (4+ spaces) are NOT filtered; indented bullets commonly contain
# wikilinks and should count as edges.
...
def included(path: Path, fm: dict) -> bool:
    ...
    try:
        ...
        resolved.relative_to(SCOPE_ROOT.resolve())
...
def extract_wikilinks(body: str) -> set[str]:
    ...
    bullets often contain wikilinks.
    """
    cleaned: list[str] = []
    ...
    for m in WIKILINK_RE.finditer(scan):
        ...
        # This matches common wikilink behavior for unique filenames.
        stem = raw.rsplit("/", 1)[-1]
        if stem:
            results.add(stem)
    return results
...
def collect_pages() -> dict[str, dict]:
    ...
    for md in sorted(WIKI_DIR.rglob("*.md")):
        ...
        if not included(md, fm):
            continue
        title_key = md.stem  # wikilinks are filename-based
        pages[title_key] = {
