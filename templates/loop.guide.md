...
| How it ends | **Fail-safe**: just don't re-arm the next wakeup — forgetting to act ends the loop | **Fail-open**: the loop must CronDelete its own cron job; a missed delete keeps firing no-op `STOP` ticks until you CronDelete the job / 7-day expiry |
...
One loop per scope (the lock enforces it). To stop it early, tell the loop to stop — it ends itself
explicitly (stops re-arming its wakeup, and CronDeletes its own recurring job if it made one). Keep the session
