...
| 06 | **NEXT ↺** | 다음 깨어남을 예약(ScheduleWakeup, prompt `<<loop.md-dynamic>>`, 1200–1800초)하거나 **생략해 종료** — 이렇게 `MAX_ITERS`나 정지 사유에서 자기종료한다. |
...
- **기능 게이트 — 먼저 스모크 테스트.** 이 템플릿이 타는 두 가지가 Claude Code 빌드에 게이팅됨(고정 버전이 아니라 롤아웃 플래그): 맨손 `/loop`의 `.claude/loop.md` 읽기, 그리고 dynamic self-pacing 깨어남(`ScheduleWakeup`) 자체. 하나라도 꺼져 있으면 `/loop`이 loop.md를 무시하거나 1회 tick 후 멈춤(깨어남이 조용히 no-op) — `MAX_ITERS`가 발동하지 않음. 야간 실행을 믿기 전 1-tick 스모크 테스트: `/loop` 실행 → 첫 턴이 이 파일(CONFIG + LEDGER)을 실제로 읽고 **다음 깨어남을 arm하는지** 확인. Claude Code 2.1.197에서 동작 확인.
