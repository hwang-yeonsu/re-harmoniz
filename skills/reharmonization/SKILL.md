...
4. **Phase C — Mutation**: decompose new `.raw/` material into existing nodes; **field-origin results (`.raw/experiments-results/`) import into the target's `## Field Evidence` with their conditions — not into Objections; external material is seed (§4)**; hunt evidence/counterexamples via web search within §6 bounds; contrast adjacent nodes → `mashups/`. **When the new material is an owner design decision (`.raw/design-decisions/`), absorb it and then sweep the experiment queue (§4 Phase C)**: for each non-`retired` `type: experiment` node, re-read its `## Decision at stake` and report — never auto-retire — any whose two branches this decision just collapsed into one outcome.
