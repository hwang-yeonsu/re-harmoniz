...
3. **Batch triage per bundle** (multiSelect, at most two passes) — the default for homogeneous bundles:
   - Pass ① **promote**: multi-select the items to act on now — adopt / reject / sharpen into a Phase B candidate; a question may be promoted to `status: escalated` when the scope's `Research escalation:` toggle is set (write its `## Escalation` block per §13: *what would change our mind*).
