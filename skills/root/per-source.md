...
3. **Atomize — decision-gated (EVOLUTION.md §1)** into `.reharm-draft/<source-stem>/`, never into `wiki/claims/`:
   ...
   - Full EVOLUTION.md §2 frontmatter with the mandatory new-node defaults: `status: seed`, `generation: 1`, `confidence: low`, `challenges_survived: 0`, `last_challenged: <today>`. Set `sources: ["[[<source-stem>]]"]` and `serves: [...]`; leave `supports: []` and `contradicts: []` **empty** — you cannot see other subs' nodes, so you cannot wire real edges.
