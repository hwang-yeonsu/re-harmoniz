...
- **Max 15 new nodes per source** (unchanged from 0.4.2; now enforced inside each sub). Beyond the cap, a sub returns `overflow_candidates` and the main agent parks them in `wiki/questions/` — 0.4.2's overflow→`questions/` behavior, preserved.
