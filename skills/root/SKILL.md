...
4. **Subs run isolated** (per `per-source.md`): each lands its source → writes `wiki/sources/<x>.md` → drafts claims into `.reharm-draft/<source-stem>/` (`status: seed`, ≤ 15 external / ≤ 3 field-origin, no web enrichment) → returns its JSON contract (the one defined in `per-source.md`). A sub never touches `index/hot/log/overview`, canonical `claims/`, or another sub's draft dir.
...
- **Max 15 new nodes per source — but 3 for a field-origin source** (a report from a declared result lane, e.g. `.raw/experiments-results/`; EVOLUTION.md §1: measurement detail lives on the source page, claims only for decision-changing findings). Beyond the cap, a sub returns `overflow_candidates` and the main agent parks them in `wiki/questions/` — 0.4.2's overflow→`questions/` behavior, preserved.
